...
- Documentation workflow
  ...
  - Flux stack scaffolding now models component-level dependencies (`namespaces -> cilium -> cert-manager -> platform components -> example-app`) instead of generic `core/platform` layers to keep sequencing explicit by technology.
  ...
  - Component-level GitOps base scaffolds now exist under `cluster/base/` (`cert-manager`, `oauth2-proxy`, `clickstack`, `otel`, `storage/minio`, `storage/ceph`, plus `apps/example`); keep `core/` and `platform/` as temporary aggregation placeholders only.
