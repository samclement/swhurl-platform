...
- `homelab-app-example-keycloak-canary` is an isolated canary route and depends on `homelab-app-example`
...
- Identity/Keycloak
  ...
  - Bitnami chart `24.2.0` defaults to `bitnami/*` image tags that are no longer pullable in this environment; pin `values.image.repository/tag` and `values.postgresql.image.repository/tag` to `bitnamilegacy/*` tags in `platform-services/keycloak/base/helmrelease-keycloak.yaml`, and keep tag versions aligned with chart defaults (`26.0.5-debian-12-r0` / `17.0.0-debian-12-r9`) to avoid runtime/env compatibility regressions.
