...
- Documentation workflow
  ...
  - Flux stack scaffolding now models component-level dependencies (`namespaces -> cilium -> cert-manager -> issuers -> platform components -> example-app`) instead of generic `core/platform` layers to keep sequencing explicit by technology.
  ...
  - Platform migration scaffolding now includes suspended Flux `HelmRelease` resources for `oauth2-proxy`, `clickstack`, `otel-k8s-daemonset`, `otel-k8s-cluster`, and `minio`; each manifest includes TODO notes to wire env/profile-driven values before unsuspending.
  - Example app migration scaffolding now includes suspended `cluster/base/apps/example/helmrelease-hello-web.yaml` targeting `charts/apps-hello`; align ingress/auth values with provider strategy before unsuspending.
