...
- `cluster/overlays/homelab/flux/stack-kustomizations.yaml` defines a Flux `dependsOn` chain (`namespaces -> cilium -> cert-manager -> issuers -> {oauth2-proxy, clickstack -> otel, storage} -> example-app`).
