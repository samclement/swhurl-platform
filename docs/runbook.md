...
Runtime Secret manifests are Git-managed and SOPS-encrypted directly at their final targets:
- `platform-services/runtime-inputs/secret-oauth2-proxy-shared.sops.yaml` -> `ingress/oauth2-proxy-shared-secret`
- `platform-services/runtime-inputs/secret-clickstack-runtime-inputs.sops.yaml` -> `observability/clickstack-runtime-inputs`
- `platform-services/runtime-inputs/secret-hyperdx.sops.yaml` -> `logging/hyperdx-secret`
...
- The `ENC[...]` values in `platform-services/runtime-inputs/*.sops.yaml` are generated by SOPS when saving plaintext edits.
...
- `CLICKSTACK_API_KEY` is stored in `secret-clickstack-runtime-inputs.sops.yaml`; use a strong random bootstrap API key unless reusing an existing ClickStack key.
- `HYPERDX_API_KEY` is stored in `secret-hyperdx.sops.yaml`; it comes from the ClickStack UI after first-login setup and can temporarily match `CLICKSTACK_API_KEY` during bootstrap.

Secret boundary:
- Keep shared platform runtime Secrets in `platform-services/runtime-inputs/*.sops.yaml`.
...
4. Update the Git-managed SOPS target Secret:
