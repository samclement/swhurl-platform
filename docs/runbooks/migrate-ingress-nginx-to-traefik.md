...
- `hubble` / `hello` / `staging-hello` return `401` when unauthenticated.
- Redirect-to-login parity with prior nginx `auth-signin` requires an additional redirect middleware pattern (tracked as follow-up).
