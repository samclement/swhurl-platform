...
- Base ingress auth is applied in overlays via shared `ingress` namespace Traefik middlewares (`oauth-signin` + `oauth-auth`) from `platform-services/oauth2-proxy/base`.
